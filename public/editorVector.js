...
function Floor() {
	...
	this.AddRoom = function() {
		if(m_tempRoom.GetPolygone().length<=2){
			alert("La Pi√®ce doit contenir plus de points !");
			return false;
		}
		m_tempRoom.SetName(prompt('Entrez le nom de la pi√®ce'));
...
function WindowCanvas() {
	...
	this.DrawGrid = function() {
		...
		// affichage du numÈro de l'Ètage
		m_ctx.fillStyle = "White";
		m_ctx.fillRect(390, 618, 110,30)
		m_ctx.fillStyle = "Black";
		m_ctx.font = '15px consolas';
		m_ctx.fillText('√âtage : ' + ("00" + m_selectedFloor).slice(-2) + "/" + ("00" + (m_floors.length-1)).slice(-2), 390, 638);
	...
	this.NewFloor = function(){
		...
		if(m_floors.length>99){
			alert("Vous ne pouvez plus faire d'√©tage")
			return;
		}
		m_selectedRoom=-1;
	...
	this.DeleteFloor = function(){
		...
		if(m_floors.length==1){
			alert("Impossible de supprimer cet √©tage");
			return;
		}
		m_selectedRoom=-1;
	...
	this.SetNameSelectedRoom = function(){
		if(m_selectedRoom == -1){
			alert("vous n'avez pas selectionn√© de pi√®ce");
			return;
		}
		this.GetSelectedFloor().GetRooms()[m_selectedRoom].SetName(prompt("Entrez le nouveau nom de la pi√®ce"));
		this.DrawSelectedFloor();
	};
	...
	// dÈsactivation du menu clique droit
	...
	this.DrawSelectedFloor = function(){
		...
		// affichage de la piËce selectionnÈ
		m_ctx.fillStyle = "white";
		m_ctx.fillRect(620, 5, 560,30)
		m_ctx.fillStyle = "Black";
		if(m_selectedRoom == -1){
			m_ctx.fillText("Pi√®ce s√©lectionn√©e : Aucune pi√®ce s√©lectionn√©", 620, 20);
		}else{
			console.log(this.GetSelectedFloor().GetRooms()[m_selectedRoom].GetName());
			m_ctx.fillText("Pi√®ce s√©lectionn√©e : " + this.GetSelectedFloor().GetRooms()[m_selectedRoom].GetName(), 620, GRID_OFFSET.y);
		}
	};
}
        ...
        case 3:
        	...
        	if(mainWindow.GetStatus() == mainWindow.GetStatusValue().DRAWING_ROOM){
        		...
        		if(mainWindow.GetSelectedFloor().AddRoom()){
            		// dÈssin de la piËce SI la piËce peut Ítre placÈ
            		mainWindow.DrawSelectedFloor();
